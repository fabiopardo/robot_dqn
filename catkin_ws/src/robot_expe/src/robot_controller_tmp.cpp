...
void RobotController::onInit(InitEvent &evt) {
  ...
  images_publisher = node_handle.advertise<sensor_msgs::Image>("camera_stream", 10);
  rewards_publisher = node_handle.advertise<std_msgs::Int32>("rewards_stream", 10);
...
double RobotController::onAction(ActionEvent &evt) {
  ...
  if (action != NOTHING) {
    ...
    switch (action) {
      ...
      case LEFT:    lvel = -0.1 * max_velocity; rvel =  0.1 * max_velocity; std::cout << "left" << std::endl;     break;
      case RIGHT:   lvel =  0.1 * max_velocity; rvel = -0.1 * max_velocity; std::cout << "right" << std::endl;    break;
