...
function trans:__init(args)
    ...
    self.nonEventProb = args.nonEventProb or 0.9 -- TODO
    self.gpu = args.gpu
    self.numEntries = 0
    self.insertIndex = 0
...
function trans:fill_buffer()
    ...
    for buf_ind=1,self.bufferSize do
        local s, a, r, s2, term = self:sample_one()
...
function trans:explore_transitions()
    local screen_height = self.images_height * 2
    local screen_width = self.images_width * 2
    local screen = torch.zeros(70, 3, screen_height, screen_width)

    for i = 1, math.min(70, self.numEntries) do
        local s, a, r, s2, t = self:get(i)
        screen[{{i}, {1},{},{}}] = image.scale(s:clone():reshape(self.images_height, self.images_width), screen_width, screen_height, "simple")
        screen[{{i}, {2},{},{}}] = image.scale(s2:clone():reshape(self.images_height, self.images_width), screen_width, screen_height, "simple")
        screen[{{i}, {3}, {}, {}}]:fill(255)
        if a == 1 then screen[{{i}, {3},{screen_height/4,screen_height/2},{screen_width/2-3,screen_width/2+3}}]:fill(0)
        elseif a == 2 then screen[{{i}, {3},{screen_height/2,3*screen_height/4},{screen_width/2-3,screen_width/2+3}}]:fill(0)
        elseif a == 3 then screen[{{i}, {3},{screen_height/2-3,screen_height/2+3},{screen_width/4,screen_width/2}}]:fill(0)
        elseif a == 4 then screen[{{i}, {3},{screen_height/2-3,screen_height/2+3},{screen_width/2,3*screen_width/4}}]:fill(0) end
        if r ~= 0 then screen[{{i}, {3},{},{}}]:mul(-1):add(255) end
    end
    win_expl_trans = image.display{image=screen, min=0, max=255, legend="transitions", win=win_expl_trans}
end
...
function trans:sample_one()
    ...
    while not valid do
        ...
        --]]
        if self.nonEventProb < 1 --[[and self.t[index+self.recentMemSize] == 0]] and self.r[index+self.recentMemSize-1] == 0 and torch.uniform() > self.nonTermProb then
...
-- buffer of bufferSize samples
-- want a batch of size batch_size (choose bufferSize to be a multiple of batch_size)
-- index from which to read in the buffer
-- if not enough remaining samples in batch, fill the buffer
-- return samples of the buffer in the range {index, index + batch_size - 1}
function trans:sample(batch_size)
    ...
    if not self.buf_ind or self.buf_ind + batch_size - 1 > self.bufferSize then
        --print("fill buffer", batch_size, self.bufferSize)
        self:fill_buffer()
    end
...
function trans:add(s, a, r, term)
    ...
    step_message = step_message .. "transition: " .. self.insertIndex .. "/" .. self.maxSize .. " "
